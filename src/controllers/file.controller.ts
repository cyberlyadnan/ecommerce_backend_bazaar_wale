...
function getBaseUrlFromRequest(req: Request): string {
  ...
  const configUrl = config.app.baseUrl?.trim();
  ...
  if (isProductionRequest && requestUrl) {
    if (isConfigLocalhost || !configUrl) {
      console.warn(
        '⚠️  APP_BASE_URL is not set or is localhost!',
        'Using detected production URL from request:',
        requestUrl,
  ...
  const port = req.app.get('port') || config.app.port;
...
export const uploadFileHandler = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    console.log('[File Upload] Environment:', config.app.env);
    console.log('[File Upload] APP_BASE_URL from config:', config.app.baseUrl);
